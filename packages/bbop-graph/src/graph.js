...
graph.prototype.remove_edge = function (subject_id, object_id, predicate_id) {
  // Ensure predicate.
  if (!predicate_id) {
    predicate_id = this.default_predicate;
  }
  ...
  if (edge) {
    ...
    if (
      this._nodes[subject_id] && // can't singleton if not there
      !this._subjects[subject_id] &&
      !this._objects[subject_id]
    ) {
      this._is_a_singleton_lookup[subject_id] = true;
    }
    // Was the object singletoned?
    if (
      this._nodes[object_id] && // can't singleton if not there
      !this._subjects[object_id] &&
      !this._objects[object_id]
    ) {
...
graph.prototype.is_topologically_equal = function (comp_graph) {
  ...
  if (base_nodes.length === comp_nodes.length && base_edges.length === comp_edges.length) {
    ...
    each(base_edges, function (base_edge) {
      var edge_p = comp_graph.get_edge(
        base_edge.subject_id(),
        base_edge.object_id(),
