...
describe("loading from JSON (good with Solr/GOlr)", function () {
  ...
  before(function () {
    ...
    var go = {
      ...
      edges: [
        ...
        { sub: "GO:0007275", obj: "GO:0032501", pred: "is_a" },
        { sub: "GO:0007275", obj: "GO:0032502", pred: "is_a" },
        { sub: "GO:0048856", obj: "GO:0032502", pred: "is_a" },
        { sub: "GO:0048869", obj: "GO:0009987", pred: "is_a" },
        { sub: "GO:0048699", obj: "GO:0022008", pred: "is_a" },
        { sub: "GO:0048869", obj: "GO:0032502", pred: "is_a" },
  ...
  it("graph two okay", function () {
    ...
    var leaf1 = leaves[0];
    var leaf2 = leaves[1];
    assert.includeMembers(["GO:0042063", "GO:0048699"], [leaf1.id()], "either of the two kids (1)");
    assert.includeMembers(["GO:0042063", "GO:0048699"], [leaf2.id()], "either of the two kids (2)");
    assert.notEqual(leaf1.id(), leaf2.id(), "the children are different");
    assert.equal(g2.get_child_nodes("GO:0022008", "part_of").length, 0, "no part_of kids");
    assert.equal(g2.get_child_nodes("GO:0022008", "is_a").length, 2, "two is_a kids");
  });
});
...
describe("failing case from the taxslim", function () {
  ...
  it("check", function () {
    ...
    assert.equal("NCBITaxon:131567", root_kids[0].id(), "and the one root kid is NCBITaxon:131567");
  });
});
...
describe("other edge access", function () {
  it("edges by subject and object", function () {
    ...
    assert.equal(g.get_edges_by_subject("d").length, 3, "d has 3 as subject");
    assert.equal(g.get_edges_by_object("d").length, 0, "d has 0 as object");

    assert.equal(g.get_edges_by_subject("z").length, 0, "z has 0 as subject");
    assert.equal(g.get_edges_by_object("z").length, 0, "z has 0 as object");

    assert.equal(g.get_edges_by_subject("y").length, 1, "y has 1 as subject");
    assert.equal(g.get_edges_by_object("y").length, 0, "y has 0 as object");

    assert.equal(g.get_edges_by_subject("n").length, 0, "n has 0 as subject");
    assert.equal(g.get_edges_by_object("n").length, 1, "n has 1 as object");
  });
});
...
describe("clone wars", function () {
  ...
  it("graph clones are perfect", function () {
    ...
    assert.deepEqual(g.metadata(), g_clone.metadata(), "clone has dupe metadata");
    assert.deepEqual(g.metadata(), { foo: "bar" }, "metadata is correct");
  });
});
...
describe("trying to work with incomplete graphs", function () {
  it("load from json, no meta", function () {
    var ng = {
      ...
      edges: [{ sub: "GO:1990387", obj: "GO:0006688", pred: "is_a" }],
    };
