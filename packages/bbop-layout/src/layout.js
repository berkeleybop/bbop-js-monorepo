...
sugiyama.partitioner = function (graph) {
    ...
    (function () {
      for (var i = 0; i < child_nodes.length; i++) {
        ...
        if (_cycle_p(cnode, call_stack)) {
          ...
          // they're already in.
          var new_edge = new sugiyama.simple_edge(cnode.id(), node.id());
          edge_set[new_edge.id()] = new_edge;
          ...
          if (!vertex_set[cnode.id()]) {
            ...
            ll(
              "to update " +
                ...
                next_level +
                "; fsr: " +
                first_seen_reference[cnode.id()] +
                "; lsr: " +
                last_seen_reference[cnode.id()],
            );
  ...
  each(edge_set, function (value, key) {
    ...
    var difference = vertex_set[edge.subject()].level - vertex_set[edge.object()].level;
    ll("diff for " + edge.subject() + " -> " + edge.object() + " = " + difference);
    ll("   " + vertex_set[edge.subject()].level + "-" + vertex_set[edge.object()].level);
      ...
      (function () {
        for (var i = 1; i <= difference; i++) {
          ...
          if (i !== difference) {
            ...
            v_id++;
            var new_v_node = new sugiyama.simple_vertex(v_node_id, true);
          ...
          var new_edge = new sugiyama.simple_edge(current_subject, current_object, true);
          edge_set[new_edge.id()] = new_edge;
        }
      })();
...
// method '.id()'.
sugiyama.bmatrix = function (object_vertex_partition, subject_vertex_partition, edge_partition) {
  // Internal logger.
  ...
  if (!object_vector || !subject_vector) {
    warn_me(
      "WARNING: We found an instance of: https://github.com/kltm/bbop-js/issues/23; using a workaround.",
    );
  }
  ...
  function getSubjectBarycenter(subject) {
    ...
    for (var o = 1; o <= object_vector.length; o++) {
      if (
        relation_matrix[object_vector[o - 1].id()] &&
  ...
  this.barycentricObjectReorder = function () {
    object_vector.sort(function (a, b) {
  ...
  this.barycentricSubjectReorder = function () {
    subject_vector.sort(function (a, b) {
    ...
    (function () {
      for (var j = 0; j < object_vector.length; j++) {
        ...
        for (var k = 0; k < subject_vector.length; k++) {
          ...
          //ll("(j: " + j + " k: " + k + ")");
          if (
            relation_matrix[object_vector[j].id()] &&
    ...
    (function () {
      for (var i = 0; i < logical_paths.length; i++) {
        ...
        for (var j = 0; j < logical_paths[i].length; j++) {
          ...
          waypoints.push({
            x: vertex_registry[cursor].x,
            y: vertex_registry[cursor].y,
          });
        }
        described_paths.push({ nodes: node_trans, waypoints: waypoints });
      }
    })();
...
var cytoscape_layout_engines = function (ngraph, layout, args) {
  ...
  var cy = cytoscape({
    elements: elements,
    headless: true, // Ensuring headless mode for server-side/non-GUI environments
    styleEnabled: false, // Not needed for pure layout calculation
  });
  ...
  each(ngraph.all_nodes(), function (n) {
    ...
    // one.
    //console.log(cy_node);
    if (cy_node && typeof cy_node.position === "function") {
      var node_x = cy_node.position().x;
      var node_y = cy_node.position().y;

      // Make sure our limits are initialized.
      if (least_x === null) {
        least_x = cy_node.position().x;
      }
      if (least_y === null) {
        least_y = cy_node.position().y;
      }
