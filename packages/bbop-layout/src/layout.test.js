...
describe("bbop-layout", function () {
  describe("bbop's sugiyama", function () {
    it("first graph", function () {
      ...
      var nodes = [n_h, n_b, n_e, n_k, n_f, n_l, n_g, n_c, n_d, n_i, n_j, n_m, n_a];
    ...
    it("this is checking the periodic layout bug", function () {
      ...
      // Correct partitioning?
      var p = new sugiyama.partitioner(g);
      assert.equal(3, p.number_of_vertex_partitions(), "node parts are should be three! (bad 1)");
      assert.equal(2, p.number_of_edge_partitions(), "edge parts should be two! (bad 1)");
      assert.equal(2, p.max_partition_width(), "max part (bad 1)");
    ...
    it("another case derived from the above", function () {
      ...
      //  e
      var bad = {
        nodes: [{ id: "d" }, { id: "e" }, { id: "b" }, { id: "a" }, { id: "c" }],
        edges: [
          { sub: "d", obj: "c" },
          { sub: "c", obj: "a" },
          { sub: "e", obj: "d" },
      ...
      // Correct partitioning?
      var p = new sugiyama.partitioner(g);
      assert.equal(4, p.number_of_vertex_partitions(), "node parts are should be four! (bad 2)");
      assert.equal(3, p.number_of_edge_partitions(), "edge parts should be three! (bad 2)");
      assert.equal(2, p.max_partition_width(), "max part (bad 2)");
  ...
  describe("checking the layout systems in cytoscape", function () {
    it("just make sure something is happening", function () {
      ...
      var zero_zero_count = 0;
      us.each(l.nodes, function (mnode) {
        // Relaxed bounds for different cytoscape versions/algorithms
        // but keeping them within reasonable prototype limits.
        assert.isAbove(mnode.x, -0.00001, "is above x");
        assert.isAbove(mnode.y, -0.00001, "is above y");
      ...
      assert.isBelow(zero_zero_count, 2, "only one node can be at 0, 0: " + zero_zero_count);
    });
  });
