...
import requests from "minerva-requests";
...
var manager = function (barista_location, namespace, user_token, engine, mode) {
  ...
  function _set_url_from_token(in_token) {
    var batch_url = barista_location + "/api/" + namespace + "/m3Batch";
    var seed_url = barista_location + "/api/" + namespace + "/seed/fromProcess";
  ...
  anchor.add_simple_composite = function (model_id, cls_expr, enabled_by_expr, occurs_in_expr) {
    ...
    if (enabled_by_expr) {
      reqs.add_type_to_individual(class_expression.svf(enabled_by_expr, "RO:0002333"), ind);
    }
    if (occurs_in_expr) {
      reqs.add_type_to_individual(class_expression.svf(occurs_in_expr, "occurs_in"), ind);
    }
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.add_individual_evidence = function (
    model_id,
    indv_id,
    evidence_id,
    source_ids,
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.add_fact_evidence = function (
    model_id,
    source_id,
    target_id,
    rel_id,
    ...
    var reqs = new request_set(anchor.user_token(), model_id);
    reqs.add_evidence(evidence_id, source_ids, with_strs, [source_id, target_id, rel_id], model_id);
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.update_annotations = function (model_id, entity, key, values, value_type) {
    var reqs = new request_set(anchor.user_token(), model_id);
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.add_individual_annotation = function (model_id, indv_id, key, value, value_type) {
    var reqs = new request_set(anchor.user_token(), model_id);
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.add_fact_annotation = function (
    model_id,
    source_id,
    target_id,
    rel_id,
    ...
    var reqs = new request_set(anchor.user_token(), model_id);
    reqs.add_annotation_to_fact(key, value, value_type, [source_id, target_id, rel_id]);
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.remove_individual_annotation = function (model_id, indv_id, key, value, value_type) {
    var reqs = new request_set(anchor.user_token(), model_id);
   ...
   * @returns {module:bbop-barista-response#response} barista response
   */
  anchor.remove_fact_annotation = function (
    model_id,
    source_id,
    target_id,
    rel_id,
    ...
    var reqs = new request_set(anchor.user_token(), model_id);
    reqs.remove_annotation_from_fact(key, value, value_type, [source_id, target_id, rel_id]);
  ...
  anchor.async_copy_individual_annotations = function (src_individual, map_ind, target_model_id) {
    ...
    if (!target_individual) {
      console.error(
        "Could not find target individual of source: ",
        src_individual,
        " in ",
        map_ind,
      );
    ...
    for (var i in src_individual.annotations) {
      ...
      var annotation_value = annotation.value.startsWith("gomodel")
        ? map_ind[annotation.value]
        : annotation.value;
      var val_type = annotation["value-type"] ? annotation["value-type"] : null;
      reqs.add_annotation_to_individual(
        annotation.key,
        annotation_value,
        val_type,
        target_individual,
  ...
  anchor.async_copy_model_annotations = function (src_model, target_model_id, new_model_title) {
    ...
    for (var i in annotations) {
      ...
      if (annotation.key == "title") {
        ...
        qp = anchor.add_model_annotation(
          target_model_id,
  ...
  anchor.async_duplicate_model = function (src_model_id, new_model_title) {
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      var copy_individual_annotations_promise = copy_model_individuals_promise.then(
        function (dup_individuals) {
      ...
      copy_model_facts_promise.then(function () {
        ...
        anchor.store_model(target_model_id).then(
          function (val) {
